...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.item == 0 || indexPath.item == 1) {
        ...
        dayVC.currentIndex = indexPath.item;
        dayVC.isToday = indexPath.item == 0 ? YES : NO;
        [self.navigationController pushViewController:dayVC animated:YES];
    }
    
    if (indexPath.item == 2 || indexPath.item == 3) {
        
        XZ_WeekViewController * weekVC = [XZ_WeekViewController new];
        weekVC.xingzuoStr = self.xingzuoStr;
        weekVC.currentIndex = indexPath.item;
        [self.navigationController pushViewController:weekVC animated:YES];
    }
    
    if (indexPath.item == 4) {
        XZ_YearViewController * yearVC = [XZ_YearViewController new];
        yearVC.xingzuoStr = self.xingzuoStr;
